...
class AuthManager
{
    ...
    public function registerUser(string $login, string $password): OperationResult
    {
        if ($this->userRepo->isLoginExist($login)) {
            return  OperationResult::createError(['message' => 'Такой логин уже существует']);
        }
    ...
    public function signinUser(string $login, string $password): OperationResult
    {
        ...
        if (!$user || !$user->verifyPass($password)) return OperationResult::createError(['message' => 'Неверный логин или пароль']);

        $this->authenticator->signIn($user);
        $this->tokenManager->generateToken($user->getId());
        return OperationResult::createSuccess(['userId' => $user->getId()]);
    }
    ...
    public function refreshUserToken(): OperationResult
    {
        ...
        if ($user === false) return OperationResult::createError(['message' => 'Не удалось пересоздать токен']);
        $this->authenticator->signIn($user);
        return OperationResult::createSuccess(['auth' => true, 'login' => $user->getLogin()]);
    }
